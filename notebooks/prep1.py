...
#Transorm data (2 classes) :
def transform_data_f_2_c(df):
    # Function to convert rating to classes
    ...
    # Replace TRUE/FALSE with 1/0 in specific columns
    df[columns_to_replace] = df[columns_to_replace].replace({True: 1, False: 0})
    
    # Split the data into features and target
    X = df.drop('rating', axis=1)  # Replace 'rating' with your target column name
    y = df['rating']  # Replace 'rating' with your target column name
    
    # Apply the rating conversion function
    y = y.apply(convert_rating_to_class)
...
# trnasorm data and make 3 classes as label
def transform_data_f(df):
    # Function to convert rating to classes
    def convert_rating_to_class(rating):
        if rating <= 3.33:
            return 0
